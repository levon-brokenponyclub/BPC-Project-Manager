...
const actorDisplayName = (p: NotificationPayloadV2): string => {
  const raw = p.actor.name || p.actor.email || "Someone";
...
const statusDetail = (p: NotificationPayloadV2): string | null => {
  if (p.change?.field === "assignee") {
    const assigneeName =
      (p.meta?.assignee_name as string | null | undefined) ?? p.change?.to;
