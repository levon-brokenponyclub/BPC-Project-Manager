...
import { TimeSkeleton } from "@/components/skeletons/TimeSkeleton";
...
export function TimePage(): React.ReactElement {
  ...
  const isPrimaryLoading =
    tasksQuery.isLoading || entriesQuery.isLoading || runningQuery.isLoading;
  const isPrimaryError =
    tasksQuery.isError || entriesQuery.isError || runningQuery.isError;
  const primaryError =
    tasksQuery.error ?? entriesQuery.error ?? runningQuery.error;
      ...
      <DataStateWrapper
        ...
        empty={
          <EmptyState
            ...
            description="Create tasks and start a timer to track work in this workspace."
          />
        }
      >
        <Card className="p-5">
