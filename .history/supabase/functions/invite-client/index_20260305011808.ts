...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!workspaceId || !email || role !== "client") {
      console.error("[invite-client] Invalid input:", { workspaceId, email, role });
      return jsonResponse({ error: "Invalid input" }, 400);
    }

    console.log("[invite-client] Checking workspace membership for user:", user.id);
    ...
    if (membershipError) {
      ...
      return jsonResponse({ error: `Membership check failed: ${membershipError.message}` }, 500);
    }
