...
function randomClientAvatarPath(): string {
  const index = Math.floor(Math.random() * CLIENT_AVATAR_PATHS.length);
  return CLIENT_AVATAR_PATHS[index];
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const selectedAvatarUrl = randomClientAvatarPath();

    async function ensureClientAvatar(userId: string) {
    ...
    if (targetUserId) {
      await ensureClientAvatar(targetUserId);
    }
