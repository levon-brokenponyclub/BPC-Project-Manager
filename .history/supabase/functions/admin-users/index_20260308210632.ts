...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Must pass the JWT explicitly — in Deno edge functions the global
    // Authorization header is not reliably picked up by auth.getUser().
    const {
      data: { user },
      error: userError,
    } = await supabaseClient.auth.getUser(forwardedUserJwt ?? undefined);
