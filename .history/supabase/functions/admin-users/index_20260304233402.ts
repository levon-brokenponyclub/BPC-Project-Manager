...
Deno.serve(async (req) => {
  ...
  try {
    ...
    console.log(`[admin-users] Token extracted: ${!!token}, length: ${token?.length}`);
    ...
    console.log(`[admin-users] getUser result - user: ${!!user}, error: ${userError?.message || 'none'}`);

    if (userError || !user) {
      console.error(`[admin-users] Auth verification failed:`, userError);
      return jsonResponse({ error: "Invalid auth token", details: userError?.message }, 401);
    }
