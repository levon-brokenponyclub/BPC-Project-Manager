-- ============================================================
-- 20260304_tasks_fields_and_uploads_PATCHED.sql
...
-- ============================================================
-- A) Upgrade public.tasks (columns)
-- ============================================================

alter table if exists public.tasks
  add column if not exists priority text not null default 'Medium',
  add column if not exists support_bucket_id uuid null,
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'tasks_support_bucket_id_fkey'
      and conrelid = 'public.tasks'::regclass
  ) then
    alter table public.tasks
      add constraint tasks_support_bucket_id_fkey
      foreign key (support_bucket_id)
      references public.support_buckets(id)
      on delete set null;
  end if;
end $$;

-- Constraints (idempotent + enforced)
do $$
begin
  if not exists (
    select 1
...
-- ============================================================
-- D) Upload system: task_files table
-- ============================================================
...
-- ============================================================
-- E) RLS helper functions (safe + stable)
-- ============================================================
...
-- Parse workspace uuid from storage object path "<workspace_id>/<task_id>/..."
...
begin
  if not exists (
    ...
    where schemaname='public' and tablename='task_files' and policyname='task_files_select_member'
  ) then
  ...
  if not exists (
    select 1 from pg_policies
    where schemaname='public' and tablename='task_files' and policyname='task_files_insert_member'
  ) then
  ...
  if not exists (
    select 1 from pg_policies
    where schemaname='public' and tablename='task_files' and policyname='task_files_delete_owner_or_admin'
  ) then
...
-- ============================================================
-- G) Storage bucket (private) + storage.objects policies
-- Convention: name = "<workspace_id>/<task_id>/<uuid>-<filename>"
-- ============================================================
...
begin
  if not exists (
    select 1 from pg_policies
    where schemaname='storage' and tablename='objects' and policyname='task_attachments_insert_member'
  ) then
    create policy task_attachments_insert_member
      ...
      with check (
        ...
        and split_part(name,'/',2) <> '' -- task_id segment exists
  ...
  if not exists (
    select 1 from pg_policies
    where schemaname='storage' and tablename='objects' and policyname='task_attachments_select_member'
  ) then
  ...
  if not exists (
    select 1 from pg_policies
    where schemaname='storage' and tablename='objects' and policyname='task_attachments_delete_owner_or_admin'
  ) then
...
-- ============================================================
-- H) Activity logging for uploads/deletes (DB-triggered)
-- ============================================================
