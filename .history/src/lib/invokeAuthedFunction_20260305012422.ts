...
export async function invokeAuthedFunction<TData = unknown>(
  ...
  if (error) {
    ...
    // Supabase FunctionsHttpError may include the underlying Response in `context`.
    ...
    if (context && typeof context.json === "function") {
      ...
      try {
        ...
        parsedMessage = serverMessage || null;
      } catch {
        // Fall through to generic message if response body cannot be parsed.
      }
