...
export async function invokeAuthedFunction<TData = unknown>(
  ...
  if (!session || isExpiringSoon) {
    console.log(`[invokeAuthedFunction] Refreshing session (missing: ${!session}, expiring: ${isExpiringSoon})`);
  ...
  console.log(`[invokeAuthedFunction] Calling ${functionName} with token (length: ${session.access_token.length})`);
