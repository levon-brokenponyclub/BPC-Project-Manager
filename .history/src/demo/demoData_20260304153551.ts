...
let notifications: Notification[] = Array.from({ length: 10 }).map((_, index) => ({
...
export async function getRunningTimer(
  ...
  return {
    entry,
    taskTitle: tasks.find((task) => task.id === runningTimer?.task_id)?.title ?? "Task",
  };
}

export async function startTimer(workspaceId: string, taskId: string): Promise<void> {
  runningTimer = {
    workspace_id: workspaceId,
    task_id: taskId,
    user_id: DEMO_USER_ID,
...
export async function stopTimer(workspaceId: string, taskId: string): Promise<void> {
  if (
    !runningTimer ||
  ...
  const durationSeconds = Math.max(
    60,
    Math.floor((new Date(endedAt).getTime() - new Date(runningTimer.started_at).getTime()) / 1000),
  );
...
export async function countUnread(workspaceId: string): Promise<number> {
  return notifications.filter(
    (notification) => notification.workspace_id === workspaceId && !notification.read_at,
  ).length;
}
...
export async function getSupportSummary(workspaceId: string): Promise<SupportSummary> {
...
> {
  ...
  return [...grouped.entries()].map(([taskId, totalSeconds]) => ({
    task_id: taskId,
    task_title: tasks.find((task) => task.id === taskId)?.title ?? "Untitled Task",
    total_seconds: totalSeconds,
    total_hours: formatDurationHours(totalSeconds),
  }));
}

export async function listSupportBuckets(
  workspaceId: string,
): Promise<
  Array<{
    id: string;
...
> {
  ...
  const periodStart = new Date(nowDate.getFullYear(), nowDate.getMonth(), 1).toISOString();
  const periodEnd = new Date(nowDate.getFullYear(), nowDate.getMonth() + 1, 0).toISOString();
