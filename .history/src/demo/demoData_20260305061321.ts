...
export async function clearAllNotifications(workspaceId: string): Promise<void> {
  notifications = notifications.filter(
    (notification) =>
      !(
        notification.workspace_id === workspaceId &&
...
export async function notifyWorkspaceEvent(
  ...
  const payloadWithActor: Record<string, unknown> = {
    ...payload,
    actor_user_id: payload.actor_user_id ?? payload.actorUserId ?? DEMO_USER_ID,
    actor_first_name:
      payload.actor_first_name ?? payload.actorFirstName ?? "Demo",
  };
