  ...
  const durationSeconds = Math.max(
    ...
    Math.round(
      (new Date(endedAt).getTime() -
        new Date((entry as TimeEntry).started_at).getTime()) /
        1000,
    ),
  );
...
export async function getHoursBreakdown(
  ...
  for (const row of data ?? []) {
    ...
    const taskTitle = ((row as { tasks?: { title?: string } }).tasks?.title ??
...
> {
  const { data, error } = await supabase
    .from("support_buckets")
    .select("id,period_start,period_end,hours_allocated,hours_used_cached")
    .eq("workspace_id", workspaceId)
    .order("period_start", { ascending: false });
