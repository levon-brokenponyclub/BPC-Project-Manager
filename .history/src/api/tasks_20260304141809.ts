...
export async function listTasks(workspaceId: string, filters: TaskFilters): Promise<Task[]> {
  let query = supabase
    .from('tasks')
    .select('*')
...
export async function createTask(
  workspaceId: string,
  input: Pick<Task, 'title' | 'description' | 'status' | 'due_date' | 'assignee_user_id'>,
): Promise<Task> {
  const {
    data: { user },
...
export async function logTaskActivity(taskId: string, type: string, payload: Record<string, unknown>): Promise<void> {
