...
export function ToastProvider({
  ...
  useEffect(() => {
    ...
    const loadUserTitle = async (): Promise<void> => {
      const {
        data: { user },
      ...
      const surname =
        (user?.user_metadata?.surname as string | undefined)?.trim() ?? "";
      ...
      const resolved = fullName || fallbackFullName || emailName || "Notification";
      if (isMounted) {
        setToastTitle(resolved);
      }
    };
        ...
        {toasts.map((toast) => (
          <div
            ...
            className={
              ...
              (toast.type === "success" ? "text-foreground" : "text-[#F6B8B8]")
            }
