...
export function SettingsPage(): React.ReactElement {
  ...
  const inviteMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!accessToken) {
        throw new Error(
          "You are not authenticated with Supabase. Please sign out and sign back in, then try again.",
        );
      }
      ...
      if (error) {
        ...
        if (notFoundHint) {
          throw new Error(
            "Invite function not found. Deploy `invite-client` to Supabase (or run Supabase local dev) and try again.",
          );
        }
      ...
      <Card className="p-5">
        ...
        <p className="mt-1 text-xs text-amber-700">
          ...
          <span className="capitalize">
            {roleQuery.isLoading ? "Loading..." : (effectiveRole ?? "(none)")}
          </span>
        </p>
      </Card>
