...
export function ClientsPage(): React.ReactElement {
  ...
  async function handleAvatarUpload(
    ...
    if (!authUser?.id) {
      showToast("You must be signed in to upload avatars.", "error");
      return;
    }
    ...
    try {
      ...
      // Keep uploader id as the first path segment so common storage RLS policies pass.
      const fileName = `${authUser.id}/${editingUser.id}-${Date.now()}.${extension}`;
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <img
                    src={
                      editAvatarUrl ||
                      fallbackAvatarFromEmail(editEmail || editingUser.email)
                    }
                    alt={editEmail || editingUser.email || "Client avatar"}
