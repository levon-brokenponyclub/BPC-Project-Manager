...
function fallbackAvatarFromEmail(email: string | null): string {
  ...
  return CLIENT_AVATAR_FALLBACKS[Math.abs(hash) % CLIENT_AVATAR_FALLBACKS.length];
}
          ...
          <tbody>
            ...
            {users.map((clientUser) => {
              ...
              const displayAvatarUrl =
                clientUser.avatar_url || fallbackAvatarFromEmail(clientUser.email);
              return (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className="w-full max-w-lg rounded-xl border border-border bg-card p-6 shadow-xl">
            <div className="mb-4 flex items-start justify-between gap-3">
              <div>
                <h2 className="text-xl font-semibold text-foreground">Edit Client Profile</h2>
                <p className="mt-1 text-sm text-muted">Update client email, name, and avatar image URL.</p>
              </div>
              <button
                type="button"
