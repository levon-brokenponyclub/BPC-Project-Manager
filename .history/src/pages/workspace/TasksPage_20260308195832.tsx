...
export function TasksPage(): React.ReactElement {
  ...
  const [subtaskParentForModal, setSubtaskParentForModal] = useState<{
    id: string;
    title: string;
  } | null>(null);
  ...
  const createTaskMutation = useMutation({
    ...
    onSuccess: async (task) => {
      ...
      if (task.parent_task_id) {
        await logTaskActivity(task.id, "subtask_created", {
          title: task.title,
          parent_task_id: task.parent_task_id,
        });
        await logTaskActivity(task.parent_task_id, "subtask_created", {
          subtask_id: task.id,
  ...
  const updateTaskMutation = useMutation({
    ...
    onSuccess: async (task, variables) => {
      ...
      if (
        task.parent_task_id &&
        variables.patch.status !== undefined &&
        variables.patch.status === "Complete"
      ) {
        await logTaskActivity(task.id, "subtask_completed", {
          parent_task_id: task.parent_task_id,
        });
        await logTaskActivity(task.parent_task_id, "subtask_completed", {
          subtask_id: task.id,
          subtask_title: task.title,
        });
      }
      ...
      if (assigneeChanged) {
        ...
        if (task.parent_task_id) {
          await logTaskActivity(task.id, "subtask_reassigned", {
            parent_task_id: task.parent_task_id,
            assignee_name: assigneeName,
          });
        }
  ...
  const deleteTaskMutation = useMutation({
    ...
    onSuccess: async ({ taskId, taskTitle }) => {
      const deletedTask =
        (tasksQuery.data ?? []).find((task) => task.id === taskId) ?? null;

      if (deletedTask?.parent_task_id) {
        await logTaskActivity(deletedTask.parent_task_id, "subtask_deleted", {
          subtask_id: taskId,
          subtask_title: taskTitle,
        });
      }
  ...
  // Sort tasks based on active sort
  ...
  useEffect(() => {
    const onDocumentMouseDown = (event: MouseEvent) => {
      ...
      const displayButtonContains =
        sortButtonRef.current?.contains(target) ?? false;
      const displayPanelContains =
        sortPanelRef.current?.contains(target) ?? false;
  ...
  useEffect(() => {
    ...
    const matchedTask = tasks.find((task) => {
      ...
      return (task.subtasks ?? []).some(
        (subtask) => subtask.id === requestedTaskId,
      );
    });

    const matchedSubtask = matchedTask?.subtasks?.find(
      (subtask) => subtask.id === requestedTaskId,
    );
    ...
    setSelectedTask(matchedSubtask ?? matchedTask);
  ...
  return (
    <div className="relative space-y-6">
          ...
          <div className="flex h-13 items-center justify-between border-b border-[#222330] px-6 py-2">
            <div className="relative">
              ...
              {filterOpen ? (
                <div
                  ref={filterPanelRef}
                  className="absolute left-0 top-full z-20 mt-2 min-w-[280px] rounded-xl border border-border/70 bg-card p-3 shadow-card"
                >
                  {/* Status Filter */}
                  <div className="mb-3">
                    ...
                    <div className="space-y-1">
                      {(
                        ...
                        <button
                          ...
                          className={`focus-ring block w-full rounded-md px-3 py-2 text-left text-sm transition-colors hover:bg-surface/60 ${
                            statusFilter === status
                  ...
                  {hasActiveFilters ? (
                    <div className="border-t border-border/50 pt-2">
                      <button
                        ...
                        className="focus-ring block w-full rounded-md px-3 py-2 text-left text-sm font-medium text-primary transition-colors hover:bg-surface/60"
                      >
                        Clear all filters
                      </button>
                    </div>
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                ...
                {sortOpen ? (
                  <div
                    ref={sortPanelRef}
                    className="absolute right-0 top-full z-20 mt-2 min-w-[220px] rounded-xl border border-border/70 bg-card p-3 shadow-card"
                  >
                    <div className="mb-2 px-2 text-xs font-semibold uppercase tracking-wide text-muted">
                      Sort by
                    </div>
                    <div className="space-y-1">
                      <button
                        ...
                        className={`focus-ring flex w-full items-center justify-between rounded-md px-3 py-2 text-left text-sm transition-colors hover:bg-surface/60 ${
                          sortBy === "status"
                            ? "bg-primary/10 text-primary font-medium"
                            : "text-foreground"
                        }`}
      ...
      <NewTaskModal
        ...
        parentTask={subtaskParentForModal}
