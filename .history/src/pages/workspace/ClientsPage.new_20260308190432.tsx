...
function RoleGroupMarker({ role }: { role: string }): React.ReactElement {
  if (role === "admin") {
    return (
...
export function ClientsPage(): React.ReactElement {
  ...
  const groupedClients = useMemo(() => {
    const groups: Record<string, GroupedClients> = {};
    ...
    users.forEach((user) => {
      ...
      workspaces.forEach((ws) => {
        ...
        // Determine role - we'll assume admins if workspace_names includes this workspace
        // This is a simplified heuristic - in reality you'd need role data per workspace
        const isVerified = Boolean(
          user.email_confirmed_at || user.confirmed_at,
        );

        if (isVerified) {
          groups[ws].clients.push(user);
        } else {
          groups[ws].clients.push(user);
        }
      });
  ...
  const isLoading = usersQuery.isLoading || workspaceQuery.isLoading;
  ...
  const handleEditProfile = (user: AdminUser) => {
    ...
    setEditWorkspaces(user.workspace_names ?? []);
    setMenuOpen(null);
  };
      ...
      <DataStateWrapper
        ...
        empty={
          <EmptyState
            ...
            description="Invite clients to get started."
            action={
              <Button
                type="button"
                variant="primary"
            ...
            <tbody>
              {groupedClients.map((group) => (
                <Fragment key={group.workspaceName}>
                    ...
                    <td className="px-6" colSpan={4}>
                      <div className="inline-flex items-center gap-[10px] text-xs">
                        ...
                        <span className="font-normal text-[#97989E]">
                          {group.admins.length + group.clients.length}
                        </span>
                      </div>
                    </td>
                  </tr>

                  {/* Admin Role Sub-group */}
                  {group.admins.length > 0 && (
                    <>
                      <tr className="h-10 border-b border-[#292B38] bg-[#191A22]">
                        <td className="px-6 pl-12" colSpan={4}>
                          <div className="inline-flex items-center gap-[10px] text-xs">
                            <RoleGroupMarker role="admin" />
                            <span className="font-medium text-[#E3E4EA]">
                              Admin
                            </span>
                            <span className="font-normal text-[#97989E]">
                      ...
                      {group.admins.map((user) => {
                        ...
                        return (
                          ...
                          >
                            <td className="px-6 py-3.5 pl-16">
                              <UserAvatar
                  ...
                  {/* Client Role Sub-group */}
                  {group.clients.length > 0 && (
                    <>
                      <tr className="h-10 border-b border-[#292B38] bg-[#191A22]">
                        <td className="px-6 pl-12" colSpan={4}>
                          <div className="inline-flex items-center gap-[10px] text-xs">
                            <RoleGroupMarker role="client" />
                            <span className="font-medium text-[#E3E4EA]">
                              Client
                            </span>
                            <span className="font-normal text-[#97989E]">
            ...
            >
              ...
              <div>
                  ...
                  {workspacesQuery.data?.map((ws) => (
                    <div key={ws.id} className="flex items-center gap-2">
                      <input
                        type="checkbox"
                        id={`workspace-${ws.id}`}
                        checked={editWorkspaces.includes(ws.name)}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setEditWorkspaces([...editWorkspaces, ws.name]);
                          } else {
                            setEditWorkspaces(
                              editWorkspaces.filter((w) => w !== ws.name),
                            );
                          }
                        }}
                ...
                <p className="mt-1 text-xs text-muted">
                  Note: Workspace assignments are shown for reference. Use the
                  workspace assignment tools to modify access.
                </p>
              </div>
