...
export function ClientsPage(): React.ReactElement {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      showToast("Client removed from workspace.");
    },
    onError: (err: unknown) => {
  ...
  // Group ALL users by workspace (including both admins and clients)
  const groupedUsers = useMemo(() => {
    const groups: Record<string, GroupedUsers> = {};

    allUsers.forEach((user) => {
      // Group all users by workspace
  ...
  const handleDelete = (userId: string) => {
    const confirmed = window.confirm(
      "Remove this client from the workspace? They will lose access but their account will not be deleted.",
    );
    if (!confirmed) return;
