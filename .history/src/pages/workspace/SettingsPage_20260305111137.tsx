...
export function SettingsPage(): React.ReactElement {
  ...
  const inviteMutation = useMutation({
    mutationFn: async () => {
      ...
      return invokeAuthedFunction("invite-client", {
        workspaceId,
        email: inviteEmail,
      ...
      <div className="space-y-6">
        <Card className="p-5">
          ...
          <p className="mt-2 text-xs text-amber-700">
            <strong>Debug:</strong> Current role: <span className="capitalize">{effectiveRole ?? "(none)"}</span>
          </p>
        </Card>

        {effectiveRole === "admin" ? (
          <Card className="p-5">
            <h2 className="text-base font-semibold text-foreground">Invite Client</h2>
            <form
            ...
            {generatedMagicLink ? (
              <div className="mt-3 space-y-2">
                <p className="text-xs text-muted">Share this link directly with the client:</p>
                <div className="flex gap-2">
                  <Input value={generatedMagicLink} readOnly />
                  <Button type="button" variant="secondary" onClick={handleCopyMagicLink}>
                    Copy
                  </Button>
                </div>
              </div>
            ...
            {inviteStatus ? <p className="mt-3 text-green-600 text-sm">{inviteStatus}</p> : null}
            {inviteError ? <p className="mt-3 text-red-600 text-sm">{inviteError}</p> : null}
          </Card>
        ) : null}

        {effectiveRole === "admin" ? (
          <Card className="p-5">
            <h2 className="text-base font-semibold text-foreground">Add Workspace</h2>
            <form
              className="mt-4 flex flex-col gap-3"
              onSubmit={(event) => {
                event.preventDefault();
            ...
            {createWorkspaceStatus ? (
              <p className="mt-3 text-green-600 text-sm">{createWorkspaceStatus}</p>
            ) : null}
