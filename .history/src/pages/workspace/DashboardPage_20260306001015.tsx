...
export function DashboardPage(): React.ReactElement {
  ...
  const isPrimaryEmpty =
    ...
    used === 0 &&
    remaining === 0;
  const visibleTasks = (recentTasksQuery.data ?? []).slice(0, 8);
  ...
  useEffect(() => {
    const onDocumentMouseDown = (event: MouseEvent) => {
      ...
      if (!displayContainerRef.current?.contains(target)) {
        ...
        <Card className="relative overflow-visible">
          <div className="flex h-10 items-center justify-between border-b border-border/70 bg-surface/70 px-6 py-2">
            <div className="relative" ref={filterContainerRef}>
              ...
              >
                <svg viewBox="0 0 16 16" className="h-4 w-4 text-muted" aria-hidden="true">
                  <path
              ...
              {filterOpen ? (
                <div className="absolute left-0 top-full z-20 mt-2 w-44 rounded-lg border border-border/70 bg-card p-2 shadow-card">
                  <button type="button" className="focus-ring block w-full rounded-md px-2 py-1.5 text-left text-sm text-foreground transition-colors hover:bg-surface/60">
                    Status
                  </button>
                  <button type="button" className="focus-ring block w-full rounded-md px-2 py-1.5 text-left text-sm text-foreground transition-colors hover:bg-surface/60">
                    Assignee
            ...
            <div className="relative" ref={displayContainerRef}>
              <button
                type="button"
              ...
              {displayOpen ? (
                <div className="absolute right-0 top-full z-20 mt-2 w-48 rounded-lg border border-border/70 bg-card p-2 shadow-card">
                  <button type="button" className="focus-ring block w-full rounded-md px-2 py-1.5 text-left text-sm text-foreground transition-colors hover:bg-surface/60">
                    Compact
                  </button>
                  <button type="button" className="focus-ring block w-full rounded-md px-2 py-1.5 text-left text-sm text-foreground transition-colors hover:bg-surface/60">
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b border-border/50 bg-surface/70 text-xs uppercase tracking-wide text-muted/80">
              <tr>
            ...
            <tbody>
              <tr>
                <td
                  className="sticky top-0 z-10 border-b border-border/50 bg-surface/80 px-6 py-2 text-sm font-medium text-foreground"
                  colSpan={5}
                >
              ...
              {visibleTasks.map((task) => {
                ...
                return (
                  <tr
                    key={task.id}
                    className="cursor-pointer border-b border-border/50 transition-colors hover:bg-card/20"
                    onClick={() => setSelectedTask(task)}
                  >
                    <td className="px-6 py-3.5">
                      <p className="max-w-[360px] truncate text-sm font-medium text-foreground">
                        {task.title}
                      </p>
                    </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-card border border-border rounded-xl shadow-xl w-full max-w-md mx-4 p-6">
            ...
            <div className="space-y-4">
              ...
              {hoursNum > 0 && (
                <div className="border border-border rounded-lg p-4 bg-surface/60">
