...
export function DashboardPage(): React.ReactElement {
  ...
  useEffect(() => {
    const onDocumentMouseDown = (event: MouseEvent) => {
      ...
      const filterPanelContains = filterPanelRef.current?.contains(target) ?? false;
      ...
      >
        ...
        <Card className="relative overflow-visible">
          ...
          <table className="w-full text-left text-sm">
            <thead className="border-b border-border/60 bg-surface/70 text-xs uppercase tracking-wide text-muted/80">
              <tr>
            ...
            <tbody>
              <tr>
                <td
                  className="sticky top-0 z-10 border-b border-border/60 bg-surface/80 px-6 py-2 text-sm font-medium text-foreground"
                  colSpan={5}
                >
                  <div className="flex items-center justify-between">
                    <div className="inline-flex items-center gap-2">
                      <span className="inline-flex h-3.5 w-3.5 rounded-full border border-yellow-500/70" />
                    ...
                    <button
                      type="button"
                      aria-label="Add item"
                      className="focus-ring inline-flex h-6 w-6 items-center justify-center rounded-md text-muted transition-colors hover:bg-card/20 hover:text-foreground"
                    >
                      +
                    </button>
                  </div>
                ...
                return (
                  ...
                  >
                    <td className="px-6 py-3.5">
                      <p className="max-w-[420px] truncate text-sm font-medium text-foreground">
                        {task.title}
                      </p>
                    </td>
