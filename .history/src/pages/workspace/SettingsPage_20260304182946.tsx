...
export function SettingsPage(): React.ReactElement {
  ...
  const workspaceQuery = useQuery({
    ...
    enabled: Boolean(workspaceId),
    onSuccess: (name) => setWorkspaceName(name),
  });
  ...
  const inviteMutation = useMutation({
    mutationFn: async () => {
      ...
      try {
        ...
        throw new Error("Unexpected response from invite function.");
      }
      ...
      {roleQuery.data === "admin" && (
        <Card className="p-5">
          ...
          >
            ...
            />
            <Button type="submit" disabled={inviteMutation.isLoading || !inviteEmail}>
              {inviteMutation.isLoading ? "Sending..." : "Send Invite"}
            </Button>
          </form>
          {inviteStatus && <p className="mt-2 text-green-600 text-sm">{inviteStatus}</p>}
      ...
      {roleQuery.data === "admin" && (
        <Card className="p-5">
          ...
          <div className="mt-3">
            ...
            {updateNameStatus && <p className="mt-1 text-green-600 text-xs">{updateNameStatus}</p>}
            {updateNameError && <p className="mt-1 text-red-600 text-xs">{updateNameError}</p>}
          </div>
        </Card>
      )}
