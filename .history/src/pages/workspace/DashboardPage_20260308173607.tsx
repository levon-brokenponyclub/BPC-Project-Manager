...
export function DashboardPage(): React.ReactElement {
  ...
  const subtasksQuery = useQuery({
    queryKey: ["tasks", workspaceId, "subtasks", selectedTask?.id ?? ""],
    queryFn: () =>
      listTasksWithUsers(workspaceId, {
        status: "All",
  ...
  const createSubtaskMutation = useMutation({
    mutationFn: async ({
      ...
      createTask(workspaceId, {
        ...
        status: "Todo",
        due_date: null,
        assignee_user_id: selectedTask?.assignee_user_id ?? null,
        priority: "Medium",
        parent_task_id: parentTaskId,
      }),
    onSuccess: async (subtask) => {
      await logTaskActivity(subtask.parent_task_id ?? subtask.id, "task_updated", {
        patch: "subtask added",
      });
      await refreshWorkspaceData();
      await subtasksQuery.refetch();
      showToast(`Created subtask \"${subtask.title || "Untitled task"}\".`);
    },
    onError: (err: unknown) => {
      const message =
  ...
  return (
    <div className="relative space-y-6">
      ...
      <TaskDrawer
        ...
        onToggleSubtaskStatus={async (subtaskId, complete) => {
          await updateSubtaskMutation.mutateAsync({ subtaskId, complete });
        }}
      />
    </div>
  );
