...
import { getEffectiveRole, getStoredRoleView, type RoleViewMode } from "@/lib/roleView";
...
async function invokeAdminUsers(
  ...
  if (!accessToken) {
    throw new Error("You are not authenticated with Supabase. Please sign in again.");
  }
...
export function ClientsPage(): React.ReactElement {
  ...
  const usersQuery = useQuery({
    queryKey: queryKeys.workspace(workspaceId).concat(["clients-users"]),
    queryFn: async () => {
            ...
            {users.map((user) => {
              ...
              return (
                <tr key={user.id} className="border-b border-border/80">
                  <td className="px-5 py-4 text-foreground">{user.email ?? "(no email)"}</td>
