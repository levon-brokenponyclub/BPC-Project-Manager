...
import type { ListItem, PhaseEntry, ProjectHealth } from "@/components/dashboard/OverviewCards";
...
export function ProjectOverviewPage(): React.ReactElement {
  ...
  const derived = useMemo(() => {
    ...
    const phaseEntries: PhaseEntry[] = parentTasks.map((task) => {
      ...
      const completeCount = subs.filter(
        (s) => s.status === "Complete",
      ).length;
      return {
        label: task.title,
    ...
    const currentIdx = phaseEntries.findIndex(
      (p) => p.label === currentPhase,
    );
    const nextMilestone =
    ...
    const atRiskHelper =
      atRiskParts.join(" · ") || "No items at risk";
          ...
          <div className="rounded-lg border border-border bg-card p-5">
            <div className="mb-4 flex items-start justify-between gap-3">
              ...
              <button
                ...
                className="inline-flex h-8 items-center justify-center whitespace-nowrap rounded-[6px] border border-border bg-surface px-3 text-[13px] font-medium text-foreground transition-colors hover:bg-surface/80"
              >
                Top Up
              </button>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              ...
              />
              <SupportMetricTile label="Used" value={`${used.toFixed(2)}h`} />
              <SupportMetricTile
                label="Remaining"
            ...
            <div className="mt-4">
              <MiniProgressBar percent={supportProgress} />
            </div>
          </div>
        </div>
      </DataStateWrapper>
