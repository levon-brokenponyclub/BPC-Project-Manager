...
export function NotificationBell({
  ...
  const getAvatarUrl = (notification: Notification): string => {
    ...
    if (payload.actor.avatar_url) {
      return payload.actor.avatar_url;
    }

    const actorUserId = payload.actor.id ?? getActorUserId(notification.payload ?? {});
    if (!actorUserId) {
  ...
  const renderNotificationRow = (notification: Notification) => {
    ...
    const formatted = formatNotification(notification.type, payload);
    const Icon = iconMap[formatted.icon] ?? Bell;
    ...
    return (
      ...
      >
        <figure className="flex shrink-0 items-center">
          ...
          <div className="-ml-2 mt-5 flex h-5 w-5 items-center justify-center rounded-full bg-card border border-border">
            <Icon className="h-3 w-3 text-muted" />
          </div>
        </figure>

        <div className="flex-1">
          <p className="text-sm font-semibold text-foreground">{formatted.title}</p>
          {formatted.subtitle ? (
            <p className="mt-0.5 text-sm text-muted">{formatted.subtitle}</p>
          ) : null}
