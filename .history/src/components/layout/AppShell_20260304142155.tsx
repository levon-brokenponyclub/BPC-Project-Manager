...
import { LayoutGrid, ListTodo, ReceiptText, Settings, Timer } from 'lucide-react'
...
export function AppShell(): React.ReactElement {
  ...
  const markReadMutation = useMutation({
    ...
    onSuccess: async () => {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: queryKeys.notifications(safeWorkspaceId) }),
        queryClient.invalidateQueries({ queryKey: queryKeys.unreadNotifications(safeWorkspaceId) }),
      ])
    },
  })
    ...
    <div className="min-h-screen bg-background p-4 md:p-6">
      <div className="mx-auto grid max-w-[1400px] grid-cols-1 gap-4 md:grid-cols-[240px_1fr]">
        <aside className="rounded-2xl border border-border bg-white px-4 py-5 shadow-soft md:min-h-[86vh]">
          <button onClick={() => navigate('/workspaces')} className="mb-7 flex items-center gap-3 text-left">
            <img src="/bpc-logo.svg" alt="BPC" className="h-10 w-10 rounded-xl" />
            <div>
              <p className="text-sm font-semibold text-foreground">Broken Pony Club</p>
              ...
              return (
                <NavLink
                  ...
                  className={`flex items-center gap-2 rounded-xl px-3 py-2 text-sm transition ${
                    active ? 'bg-primary text-white' : 'text-foreground hover:bg-stone-100'
                  }`}
                >
        ...
        <div className="space-y-4">
          <header className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-border bg-white px-4 py-3 shadow-soft">
            <TimerWidget runningTimer={runningTimerQuery.data ?? null} nowTick={tick} />
