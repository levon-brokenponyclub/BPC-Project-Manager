    ...
    <div className="min-h-screen bg-background p-4 md:p-6">
      <div className="mx-auto grid max-w-[1400px] grid-cols-1 gap-4 md:grid-cols-[240px_1fr]">
        <aside className="rounded-2xl border border-border bg-white px-4 py-5 shadow-soft md:min-h-[86vh]">
          ...
          {systemStatus.mode === "live" && systemStatus.supabaseConfigured && roleQuery.data === "admin" && (
            <div className="mt-6 rounded-xl border px-3 py-2 text-xs flex items-center gap-2"
              style={{ borderColor: systemStatus.connected ? '#22c55e' : systemStatus.lastError ? '#ef4444' : '#eab308' }}>
              <span
                ...
                title={systemStatus.connected ? "Connected" : systemStatus.lastError ? "Error" : "Not configured"}
              />
              ...
              {systemStatus.lastError && (
                <span className="ml-2 text-red-600">{systemStatus.lastError}</span>
              )}
            </div>
          )}
        </aside>
