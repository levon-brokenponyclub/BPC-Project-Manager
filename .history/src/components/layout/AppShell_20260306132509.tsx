...
export default function AppShell({
  ...
  const adminNavItems = navItems.filter((item) =>
    ["clients", "time", "reports"].includes(item.key),
  );
      ...
      >
        ...
        <div className="mt-2 flex flex-1 flex-col overflow-hidden">
          <nav className="flex flex-col gap-1">
            ...
            >
              <Inbox className="h-4 w-4 shrink-0 text-[#939496]" />
              {!collapsed ? (
                <>
          ...
          <div className="mt-4 space-y-2">
            ...
            {workspaces.length > 0 ? (
              <div className="space-y-1">
                <button
                  type="button"
                  className="focus-ring flex w-full items-center justify-between rounded-[4px] px-[6px] py-[4px] text-left text-[13px] font-medium leading-4 text-[#E2E3E5] transition-colors hover:bg-[#1A1C1F]"
                >
                  <span className="truncate">{selectedWorkspaceName}</span>
                  <ChevronDown className="h-4 w-4 shrink-0 text-[#939496]" />
                </button>
        ...
        <div className="mt-auto space-y-2 pt-3">
          {!collapsed ? (
            <button
              type="button"
              onClick={() =>ing navigate(`/w/${workspaceId}/settings`)}
              className="focus-ring flex w-full items-center gap-[10px] rounded-[4px] px-[6px] py-[4px] text-left text-[13px] font-medium leading-4 text-[#E2E3E5] transition-colors hover:bg-[#1A1C1F]"
            >
              <Settings className="h-4 w-4 shrink-0 text-[#939496]" />
