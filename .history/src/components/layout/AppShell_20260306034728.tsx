...
export default function AppShell({
  ...
  const activeNotification =
    inboxItems.find((notification) => notification.id === activeNotificationId) ??
    inboxItems[0] ??
    null;
  ...
  useEffect(() => {
    ...
    if (
      ...
      inboxItems.some((notification) => notification.id === activeNotificationId)
    ) {
      return;
    }
      ...
      <div className="relative flex min-h-screen min-w-0 flex-col overflow-hidden bg-background text-foreground">
        <header className="header sticky top-0 z-10 h-10 border-b border-border/60 bg-card/60">
          ...
          <div className="header-actions">
            ...
            <button
              type="button"
              className="focus-ring relative inline-flex h-7 items-center gap-1.5 rounded-md border border-sidebar-border/70 bg-card/20 px-2 text-xs font-medium text-sidebar-muted transition-colors hover:bg-card/35 hover:text-sidebar-foreground"
        ...
        {inboxOpen ? (
          <section className="absolute right-4 top-12 z-40 w-[min(1100px,calc(100vw-5rem))] overflow-hidden rounded-xl border border-border/70 bg-card shadow-xl">
            <div className="grid min-h-[560px] grid-cols-1 md:grid-cols-[280px_minmax(0,1fr)_260px]">
              <div className="border-b border-border/60 bg-surface/35 md:border-b-0 md:border-r">
                      ...
                      return (
                        <>
                          <div>
                            ...
                            <p className="mt-1 text-foreground">{message.status}</p>
                          </div>
                          <div>
                            <p className="text-xs uppercase tracking-wide text-muted">
                              Type
                          ...
                          <div>
                            ...
                            <p className="mt-1 text-foreground">{message.actor}</p>
                          </div>
                          <div>
                            <p className="text-xs uppercase tracking-wide text-muted">
                              Received
