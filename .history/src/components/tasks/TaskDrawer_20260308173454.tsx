                    ...
                    <div className="space-y-2">
                      {subtasks.map((subtask) => {
                        ...
                        return (
                          <div
                            key={subtask.id}
                            className="flex items-center justify-between gap-3 rounded-[4px] border border-[#292B38] bg-[#15161D] px-3 py-2"
                          >
                            <label className="flex min-w-0 items-center gap-2">
                              <input
                                ...
                                className="h-4 w-4"
                                checked={isComplete}
                                disabled={!canManageTasks || isUpdatingSubtask}
                                onChange={(event) => {
                                  const checked = event.target.checked;
                                  void onToggleSubtaskStatus(subtask.id, checked);
                                }}
                              />
                              <span
                                className={`truncate text-[13px] ${
                                  isComplete
                                    ? "text-[#97989E] line-through"
                                    : "text-white"
                                }`}
                              >
                                {subtask.title}
                            ...
                            <span className="shrink-0 text-[12px] text-[#97989E]">
                              {subtask.status}
                            </span>
                          </div>
                        );
