...
export function TaskDrawer({
  ...
  return (
    <div className="fixed inset-0 z-40 !mt-0 bg-black/20 backdrop-blur-sm">
      <aside className="fixed inset-y-0 right-0 flex h-full w-full max-w-xl flex-col border-l border-border bg-card shadow-lift animate-in slide-in-from-right">
        {/* FULL WIDTH HEADER */}
        ...
        {/* BODY: 100% HEIGHT, NAV BELOW HEADER */}
        <div className="flex min-h-0 flex-1">
          {/* MAIN CONTENT SCROLLER + LAYOUT */}
          <div className="flex min-w-0 flex-1">
            <div className="min-w-0 flex-1 overflow-y-auto px-6 pb-6 pr-4">
              {/* NEW TASK FLOW (no right nav) */}
              {isNewTask ? (
                <div className="space-y-6">
                  <section className="space-y-4">
                    ...
                    <div className="space-y-4 rounded-xl border border-border bg-stone-50/30 p-5">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div className="space-y-1">
                        ...
                        {isEditingTask && effectiveRole === "admin" ? (
                          ...
                          <div className="flex h-10 items-center rounded-xl border border-border bg-stone-50 px-3 text-sm text-muted">
                            {ownerLabel}
                          </div>
                        )}
                      </div>
              ...
              {!isNewTask && activeTab === "activity" ? (
                <div className="flex min-h-0 flex-1 flex-col">
                  ...
                  {/* Unified scroll region (activity + comments as one feed) */}
                  <div className="min-h-0 flex-1 overflow-y-auto pr-2">
                    <div className="space-y-4">
                      {/* Activity entries - muted container */}
                      {activity.length > 0 ? (
                      ...
                      <div className="space-y-3">
                        ...
                        <div className="space-y-3">
                          ...
                          {/* Quick templates as chips in a control panel */}
                        ...
                        {/* Comments list - message bubble cards */}
                        <div className="space-y-3">
                          {comments.length > 0 ? (
                            comments.map((comment) => (
                              <div
                                key={`comment-${comment.id}`}
                                className="relative rounded-2xl border border-border bg-white p-4 shadow-sm pl-4"
                              >
                                {/* Left accent bar */}
                            ...
                            <div className="rounded-2xl border border-border bg-muted/20 p-8 text-center">
                              ...
                              <div className="mt-4 flex flex-wrap justify-center gap-2">
                                {quickTemplates.slice(0, 2).map((t) => (
                                  <Button
                                    key={t.label}
                                    type="button"
                                    size="sm"
                  ...
                  {/* Composer: sticky chat bar at bottom */}
                  <div className="sticky bottom-0 border-t border-border bg-background/95 px-4 py-4 shadow-[0_-8px_24px_rgba(0,0,0,0.06)] backdrop-blur supports-[backdrop-filter]:bg-background/75">
            ...
            {/* RIGHT NAV COLUMN (sticky, below header) */}
            {!isNewTask ? (
              <div className="flex h-full w-[88px] shrink-0 flex-col border-l border-border px-3 pt-6">
                {NavPill}
              </div>
            ) : null}
          </div>
