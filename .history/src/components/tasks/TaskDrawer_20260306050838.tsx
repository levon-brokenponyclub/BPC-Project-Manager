...
export function TaskDrawer({
  ...
  return (
    <section className="fixed inset-x-0 bottom-0 top-14 z-40 overflow-hidden border-b border-border/70 bg-card">
      <div className="grid h-full min-h-0 grid-cols-1 md:grid-cols-[300px_minmax(0,1fr)_280px]">
        <div className="flex min-h-0 flex-col border-b border-border/60 bg-surface/35 md:border-b-0 md:border-r">
          <div className="flex items-center justify-between border-b border-border/60 px-3 py-2.5">
            <h3 className="text-sm font-medium text-foreground">Task Panel</h3>
            <button
              type="button"
              onClick={onClose}
          ...
          <div className="min-h-0 flex-1 space-y-1 overflow-y-auto px-3 py-2">
            {isNewTask ? (
              <div className="rounded-md border border-border/60 bg-card/40 px-3 py-2 text-sm text-muted">
                Creating a new task
              </div>
            ) : (
              navItems.map(({ key, label, Icon }) => {
                ...
                return (
                  <button
                    ...
                    className={`focus-ring flex w-full items-center gap-2 rounded-md border px-3 py-2 text-left text-sm transition-colors ${
                      isActive
                        ? "border-border/70 bg-card/60 text-foreground"
                        : "border-transparent text-muted hover:bg-card/40 hover:text-foreground"
                    }`}
                  >
        ...
        <div className="hidden min-h-0 bg-card md:flex md:flex-col">
          ...
          <div className="min-h-0 flex-1 space-y-4 overflow-y-auto px-4 py-4 text-sm">
              ...
              <p className="mt-1 text-foreground">
                {isNewTask
                  ? formatUserDisplayName(
                      workspaceUsers.find((user) => user.user_id === draftOwnerId)
                        ?.email,
                    )
                  : ownerLabel}
              </p>
            ...
            <div>
              <p className="text-xs uppercase tracking-wide text-muted">Assigned To</p>
              <p className="mt-1 text-foreground">
                {isNewTask
                  ? formatUserDisplayName(
                      workspaceUsers.find(
            ...
            {!isNewTask ? (
              <div>
                <p className="text-xs uppercase tracking-wide text-muted">Logged Time</p>
                <p className="mt-1 text-foreground">{secondsToHms(taskTimeSeconds)}</p>
              </div>
            ) : null}
          </div>
        </div>
