...
import type { Comment, Task, TaskActivity, TaskStatus, TimeEntry } from '@/types/models'
      ...
      <aside className="h-full w-full max-w-xl overflow-y-auto border-l border-border bg-white px-6 py-6 shadow-soft animate-in slide-in-from-right">
        ...
        <div className="space-y-6">
          <section className="space-y-3 rounded-2xl border border-border p-4">
            <h3 className="text-sm font-semibold uppercase tracking-wide text-muted">Task Details</h3>
            <Input
            ...
            <Textarea
              ...
              onBlur={(event) => onSaveTask(task.id, { description: event.target.value })}
            />
            <div className="grid grid-cols-2 gap-3">
              <select
                className="h-10 rounded-xl border border-border bg-white px-3 text-sm"
                value={task.status}
                onChange={(event) => onSaveTask(task.id, { status: event.target.value as TaskStatus })}
              >
                {statuses.map((status) => (
              ...
              <Input
                ...
                onChange={(event) => onSaveTask(task.id, { due_date: event.target.value || null })}
              />
            </div>
          </section>

          <section className="space-y-3 rounded-2xl border border-border p-4">
            <h3 className="text-sm font-semibold uppercase tracking-wide text-muted">Time Logged</h3>
          ...
          <section className="space-y-3 rounded-2xl border border-border p-4">
            ...
            <div className="space-y-2">
              {comments.map((comment) => (
                <div key={comment.id} className="rounded-xl bg-stone-50 p-3 text-sm">
          ...
          <section className="space-y-3 rounded-2xl border border-border p-4">
            <h3 className="text-sm font-semibold uppercase tracking-wide text-muted">Activity</h3>
