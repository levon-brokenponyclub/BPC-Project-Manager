Got you — here’s the complete file (no truncation). I kept your structure, then finished the Activity composer + Files tab (with bottom-aligned Upload), plus the new-task flow.

Paste this whole thing as TaskDrawer.tsx.
...
export function TaskDrawer({
  ...
  const NavPill = !isNewTask ? (
    <nav
      ...
      className="sticky top-28 w-16 rounded-2xl border border-border/60 bg-white/70 p-2 shadow-lift backdrop-blur-sm"
    >
        ...
        {navItems.map(({ key, label, Icon }) => {
          ...
          return (
            <div key={key} className="relative group">
              ...
              {/* Callout label: always visible for active, hover for others */}
              <span
                className={`pointer-events-none absolute right-full top-1/2 mr-3 -translate-y-1/2 whitespace-nowrap rounded-xl bg-slate-900/90 px-3 py-2 text-xs font-medium text-white shadow-md transition-opacity ${
                  isActive
                    ? "opacity-100"
      ...
      <aside className="fixed inset-y-0 right-0 flex h-full w-full max-w-xl flex-col border-l border-border bg-card shadow-lift animate-in slide-in-from-right">
        ...
        <div className="flex min-h-0 flex-1">
          ...
          <div className="flex min-w-0 flex-1">
            <div className="min-w-0 flex-1 overflow-y-auto px-6 pb-6 pr-4">
                ...
                <div className="flex flex-col gap-6">
                  <section className="space-y-4">
                    {isEditingTask && canManageTasks ? (
                      ...
                      <h2 className="break-words text-3xl font-semibold leading-tight tracking-tight text-foreground">
                      ...
                      <div className="space-y-1">
                        ...
                        <Input
                          ...
                          onChange={(event) => {
                            ...
                            void onSaveTask(task.id, { due_date: next || null });
                          }}
                          disabled={!isEditingTask || !canManageTasks}
                        />
                      </div>
              ...
              {!isNewTask && activeTab === "activity" ? (
                <div className="flex min-h-0 flex-1 flex-col">
                  ...
                  {/* Scroll region (activity + comments) */}
              ...
              {!isNewTask && activeTab === "files" ? (
                <div className="flex min-h-0 flex-1 flex-col">
                  ...
                  {/* Bottom spacer so list doesn't butt up against edge */}
                  <div className="pt-4" />
                </div>
              ) : null}
            </div>
            ...
            {!isNewTask ? (
              <div className="w-[88px] shrink-0 px-3 pt-6">
                {NavPill}
              </div>
            ) : null}
          </div>
