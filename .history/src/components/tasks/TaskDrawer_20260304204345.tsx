      ...
      <aside className="h-full w-full max-w-xl overflow-y-auto border-l border-border bg-white px-6 py-6 shadow-soft animate-in slide-in-from-right">
        ...
        <div className="space-y-6">
          <section className="space-y-3 rounded-xl border border-border p-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <select
                  ...
                  value={isNewTask ? draftStatus : task.status}
                  onChange={(event) => {
            ...
            <div className={`grid gap-3 ${isNewTask ? "grid-cols-1" : "grid-cols-2"}`}>
              <div className="space-y-1">
                <p className="text-xs font-medium text-muted">Due date</p>
                <Input
                  type="date"
                  value={isNewTask ? draftDueDate : task.due_date?.slice(0, 10) ?? ""}
                  onChange={(event) => {
