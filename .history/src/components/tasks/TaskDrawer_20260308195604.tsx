...
interface TaskDrawerProps {
  ...
  onAddSubtask?: (parentTask: TaskWithUsers) => void;
  onOpenTask?: (task: TaskWithUsers) => void;
  comments: Comment[];
              ...
              <div className="flex flex-col space-y-8">
                ...
                <section className="space-y-4 rounded-[4px] border border-[#292B38] bg-[#191A22] p-5">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    {canManageTasks && !task.parent_task_id ? (
                      <Button
                        type="button"
                        size="sm"
                        className={drawerActionButtonClass}
                  ...
                  {(task.subtasks ?? []).length > 0 ? (
                    <div className="space-y-2">
                      {task.subtasks?.map((subtask) => (
                        <div
                          key={subtask.id}
                          className="flex items-center justify-between rounded-[4px] border border-[#292B38] bg-[#15161D] px-3 py-2"
                        >
                          <button
                            type="button"
                            className="min-w-0 flex-1 truncate text-left text-[13px] font-medium text-white hover:text-primary"
                          ...
                          <div className="ml-3 flex items-center gap-2">
                            <span className="text-xs text-[#97989E]">
                              {subtask.assignee?.email
                                ? formatUserDisplayName(subtask.assignee.email)
                                : "Unassigned"}
                            </span>
                            {canManageTasks ? (
                              <select
                                ...
                                className="h-8 rounded-[5px] border border-[#25262B] bg-[#1A1B25] px-2 text-xs text-white"
                              >
                                {statuses.map((status) => (
                                  <option key={status} value={status}>
                                    {status}
