...
export function TaskDrawer({
  ...
  const assigneeLabel =
    task?.assignee?.email ||
    workspaceUsers.find((user) => user.user_id === task.assignee_user_id)?.email ||
    "Not assigned";
                ...
                <Button
                  ...
                  onClick={() => {
                    void onDeleteTask(task.id).catch(() => {
                      // no-op; deletion errors are surfaced by page-level handling
                    });
                  }}
                >
          ...
          <section className="space-y-4">
            {isNewTask || (isEditingTask && canManageTasks) ? (
              <Input
                ...
                className="h-12 border-0 bg-transparent px-0 text-3xl font-semibold tracking-tight text-foreground shadow-none focus-visible:ring-0"
              />
            ) : (
          ...
          <section className="space-y-4 rounded-xl border border-border bg-stone-50/30 p-5">
            ...
            <div className="overflow-hidden rounded-xl border border-border bg-card">
              <div className="grid grid-cols-[140px_1fr] border-b border-border">
                <div className="px-3 py-3 text-sm font-medium text-muted">Status</div>
                <div className="px-3 py-2">
                  <select
                    className="h-10 w-full rounded-xl border border-border bg-card px-3 text-sm focus-ring"
                    value={isNewTask || isEditingTask ? draftStatus : task.status}
                    onChange={(event) => {
              ...
              <div className="grid grid-cols-[140px_1fr]">
                <div className="px-3 py-3 text-sm font-medium text-muted">Priority</div>
                <div className="px-3 py-2">
                  <select
                    className="h-10 w-full rounded-xl border border-border bg-card px-3 text-sm focus-ring"
                    value={draftPriority}
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="space-y-1">
                ...
                {isNewTask || isEditingTask ? (
                  ...
                  <div className="h-10 w-full rounded-xl border border-border bg-stone-50 px-3 flex items-center text-sm text-muted">
                    {task?.assignee?.email ?? "Not assigned"}
                  </div>
                )}
              </div>
            </div>
