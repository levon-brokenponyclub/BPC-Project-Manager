...
export function TaskDrawer({
  ...
  const activateTab = (tab: TaskDrawerTab): void => {
    setActiveTab(tab);
  ...
  return (
    <div className="fixed inset-0 z-40 flex justify-end bg-black/20 backdrop-blur-sm">
      <aside className="relative h-full w-full max-w-xl overflow-y-auto border-l border-border bg-card px-6 py-6 shadow-lift animate-in slide-in-from-right">
        ...
        <div className="space-y-6">
          ...
          {!isNewTask ? (
            <section
              ref={panelSectionRef}
              className="space-y-4 rounded-xl border border-border bg-stone-50/30 p-5"
            >
              <nav
                aria-label="Task detail sections"
                className="absolute right-4 top-28 z-10 flex w-14 flex-col gap-3 rounded-2xl border border-border/60 bg-stone-50/40 p-2 shadow-md backdrop-blur-sm"
              >
                <div className="relative group">
                  <button
                    type="button"
                    aria-label="Task"
                    onClick={() => activateTab("comments")}
                  ...
                  <span className="pointer-events-none absolute right-full mr-3 top-1/2 -translate-y-1/2 rounded-xl bg-slate-900/90 px-3 py-2 text-xs font-medium text-white opacity-0 shadow-md transition-opacity group-hover:opacity-100 group-focus-within:opacity-100 whitespace-nowrap">
                    Task
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        {quickTemplates.map((template) => (
                          <Button
                            ...
                            size="sm"
                            variant="secondary"
                            onClick={() => appendToCommentBody(template.value)}
                          >
                            {template.label}
                          </Button>
                        ))}
