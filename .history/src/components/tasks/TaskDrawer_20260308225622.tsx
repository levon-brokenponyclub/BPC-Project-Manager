...
export function TaskDrawer({
  ...
  const workspaceUsersById = useMemo(
    () => new Map(workspaceUsers.map((user) => [user.user_id, user])),
    [workspaceUsers],
  );
  ...
  const resolveActorName = useCallback(
    (userId: string | null | undefined, fallbackName: string): string => {
  ...
  const resolveActorAvatar = useCallback(
  ...
  }, [activity, comments, ownerLabel, resolveActorAvatar, resolveActorName]);
          ...
          <div className="min-h-0 flex flex-col md:border-r md:border-[#222330]">
            ...
            {!isNewTask && activeTab === "activity" ? (
              <div className="flex min-h-0 flex-1 flex-col px-5 py-5">
                        ...
                        return (
                          ...
                          >
                            <div className="relative flex justify-center">
                              ...
                              {entry.type === "comment" ? (
                                <img
                                  src={entry.actorAvatarUrl || "/defaultAvatar.png"}
                                  alt={entry.actorName}
                                  className="relative z-10 mt-0.5 h-7 w-7 rounded-full border border-border bg-card object-cover"
                                  loading="lazy"
                                />
                              ) : (
                                <span className="relative z-10 mt-2 h-2.5 w-2.5 rounded-full border border-border bg-surface" />
                              )}
                            </div>
                            ...
                            {entry.type === "comment" ? (
                              ...
                              <p className="pt-0.5 text-sm leading-6 text-foreground">
                                ...
                                <span className="text-muted">{entry.actionText}</span>
                              </p>
                            )}
                ...
                <div className="sticky bottom-0 border-t border-border bg-background/95 px-1 py-4 backdrop-blur supports-[backdrop-filter]:bg-background/90">
                  <div className="flex items-start gap-3 rounded-xl border border-border bg-card p-3">
                    <img
                      src={currentUserProfile?.avatar_url || "/defaultAvatar.png"}
                      alt="Your avatar"
