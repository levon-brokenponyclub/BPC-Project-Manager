  ...
  return (
    ...
    >
      {cards.map((card, index) => (
        <article
          key={card.title}
          className={`h-full rounded-2xl border border-border/80 bg-card px-5 py-4 shadow-card transition-all duration-200 ease-out hover:-translate-y-[2px] hover:shadow-lift ${entered ? "translate-y-0 opacity-100" : "translate-y-[6px] opacity-0"}`}
        ...
        >
          ...
          <p className="mt-2 text-2xl font-semibold tracking-tight text-foreground md:text-[1.75rem]">
            <AnimatedValue value={card.value} />
          </p>

          <p
            className={`mt-1.5 text-sm text-muted ${card.truncateHelper ? "truncate" : ""}`}
